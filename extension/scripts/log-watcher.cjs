...
async function fetchLogs() {
  try {
    ...
    if (!extensionTarget) {
      console.error('❌ Extension background page not found. Make sure the extension is loaded.');
      return null;
    }
  ...
  setInterval(async () => {
    ...
    if (logs) {
      if (logs.length !== lastLogCount) {
        ...
        console.log(`✅ Updated logs: ${logs.length} entries (${logs.length - lastLogCount} new)`);
